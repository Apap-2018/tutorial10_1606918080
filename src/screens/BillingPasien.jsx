...
export class BillingPasien extends React.Component {
	...
	handleFormSubmit(e) {
		...
		Appointment.addBillingPasien(dataJson).then(response => {
			if (response.status === 200){
				...
				alert(`Sukses tagihkan pasien ${this.state.billing.pasien.nama}`)
			} else {
				this.setState({
					loading: false,
				})
				alert(`Gagal tagihkan pasien ${this.state.billing.pasien.nama}`)
			}
		})
	}
	...
	render() {
		if (this.state.loading) {
			...
			return (
				<FormBillingPasien billing={this.state.billing} onSubmit={this.handleFormSubmit} />
			)
		}
	}
}